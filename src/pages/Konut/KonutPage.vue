  ...
  <q-layout view="lHh Lpr lFf"  >
    <q-header reveal elevated  >
      <q-toolbar>
        ...
        <q-toolbar-title class="text-subtitle2 text-bold text-center">Konut</q-toolbar-title>
      </q-toolbar>
    </q-header>
    <q-page-container>
  <div >
        ...
        <q-form
         ...
         >
          ...
          <q-input
            ...
            hide-bottom-space
            lazy-rules
            :rules="[ val => val && val.length > 0 || 'Lutfen Şasi No giriniz']"
          />
          <q-input
            dense
            outlined
            ...
            hide-bottom-space
            lazy-rules
            :rules="[ val => val && val.length > 7 || 'Lutfen E-Posta Adresi giriniz']"
          />
          <q-select
            outlined
...
<!--            <q-btn @click="() => { done1 = true; step = 2 }" type="submit" color="primary" label="Ilerle" no-caps class="full-width" />-->
            <q-btn  type="submit" color="primary" label="İlerle" no-caps class="full-width" />
          </q-stepper-navigation>
        </q-form>
        ...
        >
          ...
          <div class="row no-wrap justify-between">
            <div class="col-3 q-my-auto">
              <label class="text-no-wrap  text-center ">Bina İnşa Yılı</label>
            </div>
            <div class=" col-9 row">
              <div class="col-5">
                <q-input
          ...
          <q-input
            ...
            hide-bottom-space
            lazy-rules
            :rules="[ val => val && val.length > 0 || 'Lutfen bina bedeli giriniz']"
          />
          <q-input
            dense
            outlined
            ...
            hide-bottom-space
            lazy-rules
            :rules="[ val => val && val.length > 0 || 'Lutfen  eşya bedeli giriniz']"
          />
          <q-input
            dense
            outlined
            ...
            hide-bottom-space
            lazy-rules
            :rules="[ val => val && val.length > 0 || 'Lutfen  cam bedeli giriniz']"
          />
          <q-input
            dense
            outlined
            ...
            hide-bottom-space
            lazy-rules
            :rules="[ val => val && val.length > 0 || 'Lutfen  cevab giriniz']"
          />
          ...
          />
          <q-stepper-navigation>
            <q-btn  type="submit" color="primary"   label="Teklif Al" no-caps class="full-width" />
         ...
         <div class="col-11  text-grey-8" style="font-size: 11px">
           <b>Uyarı:</b> Bir sigorta poliçesinin
           düzenlenmesi amacı ile beyan edilen bilgilerin
           eksiklik içermesi ve/veya yanlış olması ve/veya
           yanıltıcı beyan içermesi halinde, işbu sigorta
           poliçesi ile ilgili şirketimizin her hangi
           bir sorumluluğu bulunmayacak ve/veya
           poliçe geçersiz kılınabilecektir.
         </div>
            <div class="col-12">
              <q-separator spaced/>
            </div>
...
let aracTarzOptions = ref(aracTarz.value)
...
const filterAracModel = (val: any, update: any) => {
  ...
  update(() => {
    const needle = val.toLowerCase()
    aracModelOptions.value = filteredArachModel.filter(v => v.Tip_Adi.toLowerCase().indexOf(needle) > -1)
  })
}
const filterAracTarz = (val:any,update:any) =>{
  if (val  === ''){
    update(() => {
      aracTarzOptions.value = aracTarz.value
    })
    return
  }
  update(() => {
    const needle = val.toLowerCase()
    aracTarzOptions.value = aracTarz.value.filter(v => v.arac_tipi.toLowerCase().indexOf(needle) > -1)
  })
}
const filterRenk = (val:any,update:any) =>{
  ...
  update(() => {
    const needle = val.toLowerCase()
    renkOptions.value = renk.value.filter(v => v.adi.toLowerCase().indexOf(needle) > -1)
  })
}
const filterIl = (val:any,update:any) =>{
  ...
  update(() => {
    const needle = val.toLowerCase()
    ilOptions.value = il.value.filter(v => v.Aciklama.toLowerCase().indexOf(needle) > -1)
  })
}
const filterIlce = (val:string,update:any) =>{
  ...
  update(() => {
    const needle = val.toLowerCase()
    ilceSelectGetOptions.value = ilce.value.filter(v => v.ilce_Adi.toLowerCase().indexOf(needle) > -1)
  })
}
const filterBelediye = (val:string,update:any) =>{
  ...
  update(() => {
    const needle = val.toLowerCase()
    belediyeSelectGetOptions.value = belediyeSelect.value.filter(v => v.Belediye_Adi.toLowerCase().indexOf(needle) > -1)
  })
}
const filterMahalle = (val:string,update:any) =>{
  ...
  update(() => {
    const needle = val.toLowerCase()
    mahalleSelectOptions.value = mahalleSelect.value.filter(v => v.Mahalle_Adi.toLowerCase().indexOf(needle) > -1)
  })
}
const filterSokak = (val:string,update:any) =>{
  ...
  update(() => {
    const needle = val.toLowerCase()
    sokakSelectOptions.value = sokakSelect.value.filter(v => v.Sokak_Adi.toLowerCase().indexOf(needle) > -1)
  })
}
const filterAgent = (val:string,update:any) =>{
  ...
  update(() => {
    const needle = val.toLowerCase()
    agentOptions.value = agent.value.filter(v => v.Acente_Adi.toLowerCase().indexOf(needle) > -1)
  })
}
...
const aracMarkaUpdated = (id?:number) => {
  ...
  let filteredModels = aracModelSelect.value.filter((item) => item.Marka_Kod === marka?.Marka_Kod)
  filteredArachModel =  filteredModels
}

const getIlOnSelect = (data:object) => {
    ilceSelectGetOptions.value = ilce.value.filter((item) => item.il_Kodu === data?.id)
}
const getIlceOnSelect = (data:object) => {
  belediyeSelectGetOptions.value = belediyeSelect.value.filter((item) => item.ilce_Kodu === data?.id)
}
const getBelediyeOnSelect = (data:object) => {
  mahalleSelectOptions.value = mahalleSelect.value.filter((item) => item.belediyenin_Kodu === data?.id)
 }
const getMahalleOnSelect = (data:object) => {
  sokakSelectOptions.value = sokakSelect.value.filter((item) => item.mahallenin_Kodu === data?.id)
}
const onNextStep = () => {
  step.value++
}
